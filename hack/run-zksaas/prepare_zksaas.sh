...
unzip -o /tmp/zksaas.zip -d /tmp/
rm /tmp/zksaas.zip
cd /tmp/zksaas

CARGO_HTTP_MULTIPLEXING=false RUSTFLAGS="-Ctarget-cpu=native --cfg tokio_unstable -Awarnings" cargo +nightly build --release --example plonk_bench