...
pub async fn c_acc_product_and_share<F: FftField, Net: MPCSerializeNet>(
    ...
    let masked_shares = shares
        ...
        .collect::<Vec<_>>();
    end_timer!(_timer);
    let masked_x = join_all(masked_shares.chunks_exact(block_size).enumerate().map(
        |(i, masked_shares)| async move {
    ...
    let subtree_vx0_share: Vec<Vec<F>> = transpose(
        subtree_to_share
            .iter()
            .step_by(2)
            .map(F::clone)
    ...
    let subtree_vx1_share: Vec<Vec<F>> = transpose(
        subtree_to_share
            .iter()
            .skip(1)
            .step_by(2)
    ...
    let subtree_v1x_share: Vec<Vec<F>> = transpose(
        subtree_to_share
            .iter()
            .skip(subtree.len() / 2)
            .map(F::clone)
    ...
    let compute_leader_tree_share_timer =
        start_timer!("Leader: Compute leader tree share", net.is_leader());
    ...
    if net.is_leader() {
        ...
        while layer_len > 0 {
            for j in 0..party_count {
                leader_tree
                    .extend_from_slice(&leader_receiving[j][start_index..start_index + layer_len]);
            }
...
pub async fn d_acc_product<F: FftField, Net: MPCSerializeNet>(
    ...
    *subtree.last_mut().unwrap() = F::ZERO;
    ...
    let ld_receive_timer = start_timer!("Send elements to leader", net.is_leader());
    let leader_receiving = net
        .worker_send_or_leader_receive_element(&subtree[subtree.len() - num_to_send], sid)
        .await
        .unwrap();
    end_timer!(ld_receive_timer);
    ...
    if net.is_leader() {
        let leader_receiving = leader_receiving.unwrap();
        let ld_compute_timer = start_timer!("Leader: Compute leader tree", net.is_leader());
        let leader_tree_len = party_count;
        let mut leader_tree = leader_receiving;
        // Now leader has the bottem of the leader tree.
        // Leader uses N elements (each is a root of a subtree) to calculate remaining layers.
        // NOTE: We do not guarantee correctness here.
        for i in leader_tree_len..(leader_tree_len * 2 - 1) {
            let (x0, x1) = sub_index(i);
            leader_tree.push(leader_tree[x0] * leader_tree[x1]);
        }
        leader_tree.push(F::ZERO);
...
mod tests {
    ...
    fn acc_product_test() {
        ...
        assert_eq!(
            res_0,
            vec![Fr::from(1), Fr::from(3), Fr::from(2), Fr::from(24)]
        );
        assert_eq!(
            res_1,
            vec![Fr::from(2), Fr::from(4), Fr::from(12), Fr::from(0)]
        );
        assert_eq!(
            res_2,
            vec![Fr::from(2), Fr::from(12), Fr::from(24), Fr::from(0)]
        );
    }
}
